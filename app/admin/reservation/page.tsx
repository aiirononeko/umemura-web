...
const ReservationDetail = (props: { reservation: Reservation }) => {
  ...
  return (
    <>
      ...
      <h5>電話番号</h5>
      <h1>{reservation.customerPhoneNumber}</h1>
      <h5>メールアドレス</h5>
      <h1>{reservation.customerEmail}</h1>
    </>
  );
};
...
export default function Top() {
  ...
  useEffect(() => {
    (async () => {
      if (uid != undefined) {
        const reservationDocuments = (await getDocuments(
          "reservations"
        )) as Reservation[];
  ...
  useEffect(() => {
    const filteredDocuments = originReservations
      .filter((r) => {
        const date = r.date.toDate();
        const tDate = targetDate.toDate();
        const year = date.getFullYear();
  ...
  return (
    <Container>
      <Modal opened={opened} onClose={close} size="xs">
        <ReservationDetail reservation={reservations.at(num)!} />
      </Modal>
      <Center mb="lg">
        <Title>予約管理日時</Title>
      </Center>
      <Center mb="lg">
        <Button component={Link} href="/admin/availableTime" className="mt-4">
          予約可能日時を登録
        </Button>
      </Center>
      <Center>
